...
int counter(int map[], const int button[][5], int count, int depth) {
    if (depth == 10) {
        for (int i = 0; i < 16; i++) {
            if (map[i] != 0) {
                return 987654321;
    ...
    int temp = 987654321;
    for (int j = 0; j < 4; j++) {
        temp = std::min(counter(map, button, count + j, depth + 1),temp);
        for (int i = 0; i < 5; i++) {
            int button_num = button[depth][i];
            if (button_num == 16) {
                continue;
            }
            map[button_num] = (map[button_num] + 1) % 4;
        }
    }
    return  temp;
}
...
int main() {
    int cases;
    std::cin >> cases;
    const int button[10][5] = {{0, 1,  2,  16, 16},
                               {3, 7,  9,  11, 16},
                               {4, 10, 14, 15, 16},
                               {0, 4,  5,  6,  7},
                               {6, 7,  8,  10, 12},
