...
int getError(int start, int length) {
    int minNum = map[start];
    int maxNum = map[start + length - 1];
    int tempMin = INF;
    for (int i = minNum; i <= maxNum; i++) {
        ...
        tempMin = min(tempSum, tempMin);
    }
    return tempMin;
}

int quantization(int start, int left) {
    if (start == n) {
        return 0;
    }
    if (left == 0 && start != n) {
    ...
    int &tg = cache[start][left];
    if (tg != -1) {
        return tg;
    }
    int tempMin = INF;
    for (int i = 1; i <= n - start; i++) {
        tempMin = min(tempMin, getError(start, i) + quantization(start + i, left - 1));
    }
    return tg = tempMin;
}
