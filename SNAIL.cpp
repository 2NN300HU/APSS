...
double climb(int well, int rain) {
    if (well <= 0) return 1;
    if (rain == 0) return 0;
    double &ret = cache[well][rain];
    if (ret != -1) {
        return ret;
    }
    return ret = 0.75 * climb(well - 2, rain - 1) + 0.25 * climb(well - 1, rain - 1);
}
...
int main() {
    ...
    for (int c = 0; c < cases; c++) {
        ...
        int well, rain;
        cin >> well >> rain;
        cout << fixed << setprecision(10) << climb(well, rain) << endl;
    }
    return 0;
}